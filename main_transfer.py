...
parser.add_argument('--output', default='/home/hexiang/DomainAdaptation_DVS/Results_rebuttal/', type=str, metavar='PATH',
...
parser.add_argument('--domain-loss-coefficient', type=float, default=0.5,
...
# camvis
parser.add_argument('--camvis', action='store_true',
                    help='cam vis')

parser.add_argument('--tsregular', default=-1.0, type=float,
                    help='temporal spatial regularization coeffi')

parser.add_argument('--pow', default=3, type=int,
                    help='power of replacement')
...
def main():
    ...
    if args.local_rank == 0:
        ...
        exp_name = '-'.join([
            ...
            "TET_loss_{}".format(args.TET_loss),
            "hsv_{}".format(not args.no_use_hsv),
            "sl_{}".format(not args.no_sliding_training),
            "regularization_{}".format(args.regularization),
            "pow_{}".format(args.pow)
        ])
    ...
    try:  # train the model
        ...
        for epoch in range(start_epoch, args.epochs):
            ...
            if epoch >= (args.epochs / 2) and args.camvis is True:
                break
...
def train_epoch(
    ...
    #     set_MaxReplacement_epoch = 0.8 * args.epochs

    global source_input_list, source_label_list, CALTECH101_list, ImageNet_list,  mnist_input_list, CEPDVS_list
    for batch_idx, (inputs, label) in enumerate(target_loader):
        P_Replacement = ((batch_idx + epoch * batch_len) / (set_MaxReplacement_epoch * batch_len)) ** args.pow
        P_Replacement = P_Replacement if P_Replacement <= 1.0 else 1.0
        if args.no_sliding_training:
            P_Replacement = 0.0
            if epoch > args.epochs / 2:
                P_Replacement = 1.0
        # if args.target_dataset == "NCALTECH101" and args.domain_loss is True and args.no_use_hsv and args.no_sliding_training:  # 仅仅为一组NCALTECH101消融实验设置
        ...
        if args.target_dataset == "nmnist":
            ...
            for idx, label_sampler in enumerate(sampler_list):
                tmp_sampler_list.append(torch.randint(mnist_input_list[label_sampler],
                                                      mnist_input_list[label_sampler + 1], (1,)).item())
        elif args.target_dataset == "NCALTECH101":
            tmp_sampler_list = []
            for idx, label_sampler in enumerate(sampler_list):
                tmp_sampler_list.append(torch.randint(CALTECH101_list[label_sampler][0],
                                                      CALTECH101_list[label_sampler][0] + 1 + int((CALTECH101_list[label_sampler][1] - CALTECH101_list[label_sampler][0]) * args.rgbdata_ratio), (1,)).item())
            ...
            sampler_list = torch.tensor(sampler_list) * 20 + torch.randint(0, 15, (len(sampler_list),))
        ...
        with amp_autocast():
            ...
            for t in range(args.step):
                t_dlloss = (1 - torch.abs(CKA.linear_CKA(domain_rbg_list[t].view(args.batch_size, -1), domain_dvs_list[t].view(args.batch_size, -1))))
                if args.regularization:
                    if args.tsregular > 0.0:
                        domain_loss += args.tsregular * t_dlloss
                    else:
                        domain_loss += model.coefficient[t].sigmoid() * t_dlloss
                else:
                    domain_loss += t_dlloss
            domain_loss /= args.step

            # compute cls loss
            lamb = 1e-3
            if args.TET_loss:
                loss_rgb = 0
                rgb_tet_loss_first = 0
                rgb_tet_loss_second = 0
                dvs_tet_loss_first = 0
                dvs_tet_loss_second = 0
                assert len(output_rgb) == len(output_dvs)

                # first term
                for i in range(len(output_rgb)):
                    rgb_tet_loss_first += loss_fn(output_rgb[i], label)
                    if args.regularization:
                        if args.tsregular > 0.0:
                            dvs_tet_loss_first += (1 - args.tsregular) * loss_fn(output_dvs[i], label)
                        else:
                            dvs_tet_loss_first += (1 - model.coefficient[i].sigmoid()) * loss_fn(output_dvs[i], label)
                    else:
                        dvs_tet_loss_first += loss_fn(output_dvs[i], label)
                rgb_tet_loss_first /= len(output_rgb)
                dvs_tet_loss_first /= len(output_dvs)

                # second term
                t_output_rgb = torch.stack(output_rgb, dim=0)
                t_output_dvs = torch.stack(output_dvs, dim=0)
                secondLoss = torch.nn.MSELoss()
                for i in range(len(output_dvs)):
                    y = torch.zeros_like(t_output_dvs[i]).fill_(args.threshold)
                    if args.regularization:
                        dvs_tet_loss_second += (1 - model.coefficient[i].sigmoid()) * secondLoss(t_output_dvs[i], y)
                    else:
                        dvs_tet_loss_second += secondLoss(t_output_dvs[i], y)
                    rgb_tet_loss_second += secondLoss(t_output_rgb[i], y)
                rgb_tet_loss_second /= len(output_rgb)
                dvs_tet_loss_second /= len(output_dvs)

                loss_rgb = (1 - lamb) * rgb_tet_loss_first + lamb * rgb_tet_loss_second
                loss_dvs = (1 - lamb) * dvs_tet_loss_first + lamb * dvs_tet_loss_second
    ...
    return OrderedDict([('loss', losses_m.avg), ('domainLoss', domain_losses_m.avg),
